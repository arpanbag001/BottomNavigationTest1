 ...
 */
public class CustomBottomNavigationMenuView extends ViewGroup implements MenuView {
    ...
    private final Pools.Pool<CustomBottomNavigationItemView> mItemPool = new Pools.SynchronizedPool<>(5);

    private boolean mShiftingMode = true;

    private CustomBottomNavigationItemView[] mButtons;
    ...
    public CustomBottomNavigationMenuView(Context context) {
        this(context, null);
    }

    @SuppressLint("RestrictedApi")
    public CustomBottomNavigationMenuView(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public void setIconTintList(ColorStateList tint) {
        ...
        for (CustomBottomNavigationItemView item : mButtons) {
    ...
    public void buildMenuView() {
        ...
        mButtons = new CustomBottomNavigationItemView[mMenu.size()];
        mShiftingMode = mMenu.size() > 3;
        for (int i = 0; i < mMenu.size(); i++) {
            mPresenter.setUpdateSuspended(true);
            mMenu.getItem(i).setCheckable(true);
            mPresenter.setUpdateSuspended(false);
            CustomBottomNavigationItemView child = getNewItem();
            mButtons[i] = child;
    ...
    private CustomBottomNavigationItemView getNewItem() {
        CustomBottomNavigationItemView item = mItemPool.acquire();
        if (item == null) {
            item = new CustomBottomNavigationItemView(getContext());
        }
        return item;
    }
